...
export const stream: StreamFunction<"openai-completions", OpenAICompletionsOptions> = (
	...
	(async () => {
		...
		try {
			...
			// Some OpenAI-compatible providers (DeepSeek-R1, Qwen, MiniMax, vLLM)
			// embed reasoning in the content stream as a leading
			// `<think>...</think>` segment instead of a dedicated reasoning
			// field. Route that segment into a thinking block so it never
			// pollutes assistant text or downstream structured-output parsing.
			const THINK_OPEN = "<think>";
			const THINK_CLOSE = "</think>";
			let thinkPhase: "detect" | "thinking" | "answer" = "detect";
			let thinkBuf = "";
			let answerStarted = false;

			const appendAnswerText = (text: string) => {
				if (text.length === 0) return;
				// Strip the newline run models emit right after `</think>`.
				const piece = answerStarted ? text : text.replace(/^\s+/, "");
				answerStarted = true;
			...
			const appendContent = (delta: string) => {
				if (thinkPhase === "answer") {
					appendAnswerText(delta);
					return;
				}
				thinkBuf += delta;
				if (thinkPhase === "detect") {
					const trimmed = thinkBuf.replace(/^[ \t\r\n]+/, "");
					if (trimmed.length === 0) return; // leading whitespace — keep waiting
					if (trimmed.startsWith(THINK_OPEN)) {
						thinkBuf = trimmed.slice(THINK_OPEN.length);
						thinkPhase = "thinking";
					} else if (THINK_OPEN.startsWith(trimmed)) {
						return; // could still become `<think>` once more bytes arrive
					} else {
						thinkPhase = "answer";
						const buffered = thinkBuf;
						thinkBuf = "";
						appendAnswerText(buffered);
						return;
					}
				}
				const closeIndex = thinkBuf.indexOf(THINK_CLOSE);
				if (closeIndex !== -1) {
					appendThinkingText(thinkBuf.slice(0, closeIndex));
					const rest = thinkBuf.slice(closeIndex + THINK_CLOSE.length);
					thinkBuf = "";
					thinkPhase = "answer";
					appendAnswerText(rest);
					return;
				}
				// Hold back a tail that could still be a partial `</think>` prefix;
				// it is flushed when the tag completes or the stream ends.
				const safe = Math.max(0, thinkBuf.length - (THINK_CLOSE.length - 1));
				appendThinkingText(thinkBuf.slice(0, safe));
			...
			const flushContent = () => {
				...
				const buffered = thinkBuf;
				thinkBuf = "";
				if (thinkPhase === "thinking") appendThinkingText(buffered);
				else appendAnswerText(buffered);
			};
